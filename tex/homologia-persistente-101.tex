...
definir o complexo simplicial Alpha Shape.
...
\begin{defi}
  Seja $X$ um conjunto de pontos $\Set{x_1, \dots, x_n}$ em $\mathbb{R}^d$. O complexo
  de Vietoris-Rips de X para um valor real $r>0$ é o conjunto $A^r(X)$, onde o simplexo
  $\sigma = <x_{i_1}, \dots, x_{i_k}> \in A^r(X)$ se, e somente se vale a seguinte
  condição
  \begin{equation*}
...
Na \autoref{fig:alph_ex} temos o exemplo de um complexo Alpha Shape. É interessante
notar que o Alpha Shape é um subcomplexo do complexo de \v{C}ech, ou seja,
...
posição geral, então o complexo simplicial abstrato Alpha Shape pode ser
realizado geometricamente no $\mathbb{R}^d$, ou seja o complexo simplicial geométrico pode ser construído
no $\mathbb{R}^d$! Isso é fundamental computacionalmente, já que diminui a complexidade
dos cálculos e aumenta a velocidade para obtenção do complexo.

\begin{figure}[htb]
  \centering
  \includegraphics[width=0.7\textwidth]{images/placeholder.png}
  \caption{Complexo Alpha Shape para um conjunto de pontos no plano.}
  \label{fig:alph_ex}
\end{figure}

Uma variação muito importante do Alpha Shape é o Alpha Shape com peso. Ao invés
de considerar um raio fixo para cada bola ao redor de um ponto, podemos dar
um \textit{peso} para cada ponto. Seja $X=\Set{x_1, \dots, x_n}$ e
$\Set{w_1, \dots, w_n}$ conjunto de valores maiores ou iguais a zero. Para
cada $x_i$, ao invés de associar a bola usual do complexo de Alpha Shape, associamos
a seguinte bola.
\begin{equation*}
...
entre os espaços vetoriais $H_p(K_i)$ e $H_p(K_j)$.
...
dela. Denote por $R$ a matriz reduzida de $\partial$. 
